...
class fs_updater
{
   ...
   {
      $urls = array(
          'https://github.com/sartin1/peluqueria/archive/master.zip',
          'https://codeload.github.com/sartin1/peluqueria/zip/master'
      );
      ...
      foreach($urls as $url)
      {
         if( @file_put_contents('update.zip', $this->curl_get_contents($url)) )
         {
            ...
            if($zip_status === TRUE)
            {
               ...
               $this->recurse_copy('peluqueria-master/', '.');
               $this->delTree('peluqueria-master/');
